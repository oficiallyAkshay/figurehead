...
Then choose a `style`. Every hero gets one on purpose; `style` is never left out of the spec, and `flat` is never chosen just because it is the default. Read `references/style.md` for what each named style draws and where it fits, starting with "Choosing a world". Use `flat` only for compatibility, when the hero already renders flat and must not move, such as readmerlin's and tidy-inbox's goldens. Otherwise, find the style in the world the product's own name or job gives: pierless is a harbour, so `chart`.

When no existing style fits that world, say so in chat: name the world in one sentence and wait for the owner to accept it before drawing anything. A receipts tool's own feeling is efficiency, structure and time back, not water, so its world is its own two app surfaces, an inbox list and a calendar month with the trip dates highlighted, in an interface register with one accent and no texture, not `chart`; that world was proposed and accepted before anything was drawn. Naming a world is not the same as naming a style: a style earned this way, `window` for the receipts tool, is added to the renderer properly, with its own name and its own golden, per `references/style.md`'s "earning a new style" section, and it is being added to the renderer now, not yet one to choose. Today only `chart` and `flat` are in the renderer. A style is never faked by reusing the nearest existing one's scene with new words.
